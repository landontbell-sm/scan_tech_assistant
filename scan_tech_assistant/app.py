...
from nasl_regex import parse_file, PLUGIN_ID_RE
from build_index import find_plugin, resolve_includes
...
templates = Environment(loader=FileSystemLoader("prompts"),trim_blocks=True,lstrip_blocks=True,)

# Welcome message when the chat starts
@cl.on_chat_start
async def start():
    await cl.Message(content="**Scan Agent:** Enter a numeric Nessus plugin ID and I'll pull the plugin details and walk through how to validate it by hand.").send()
...
async def main(message: cl.Message):
    ...
    if not match:
        await cl.Message(content="Please enter a valid Nessus plugin ID (a number).").send()
        return
    ...
    try:
        ...
        raw_content = open(plugin_path, "r", encoding="utf-8", errors="replace").read()
        ...
        async with cl.Step(
            name=os.path.basename(plugin_path), type="tool", language="nasl", default_open=False
        ) as source_step:
        ...
        messages = [{
            ...
            "content": [
                ...
                {"type": "text", "text": source_block, "cache_control": {"type": "ephemeral"}},
            ],
        }]
        ...
        async with cl.Step(name="Building testing procedure", type="tool"):
            async with client.messages.stream(
                ...
                output_config={
                    "effort": "medium",
                    "format": {"type": "json_schema", "schema": ProcedureResponse.model_json_schema()},
                },
                messages=messages,
            ) as stream:
        ...
        if response.stop_reason == "refusal":
            await cl.Message(content="Claude declined to answer for this plugin (safety refusal).").send()
            return
        text_block = next((b for b in response.content if b.type == "text"), None)
        if text_block is None:
            await cl.Message(content=f"No answer produced (stop_reason={response.stop_reason}).").send()
            return
        procedure = ProcedureResponse.model_validate_json(text_block.text)

        show_target_legend = any(step.command and "<TARGET>" in step.command for step in procedure.steps)
